...
class ModuloAgenda:
    def tela_agenda(self):
        ...
        header = ctk.CTkFrame(scroll, fg_color="transparent"); header.pack(fill="x", pady=(0, 20))
        ctk.CTkLabel(header, text="Agendamentos", font=("Arial", 24, "bold")).pack(side="left")
        badge = ctk.CTkFrame(header, fg_color="white", corner_radius=20, border_width=1, border_color="#E2E8F0"); badge.pack(side="right")
        ctk.CTkLabel(badge, text="Fevereiro 2026", font=("Arial", 14, "bold")).pack(padx=20, pady=5)
        dias_frame = ctk.CTkFrame(scroll, fg_color="white", corner_radius=20, border_width=1, border_color="#E2E8F0"); dias_frame.pack(fill="x", pady=(0, 30))
        ...
        for i, (nome, num) in enumerate(dias):
            d_card = ctk.CTkFrame(dias_frame, fg_color="transparent"); d_card.grid(row=0, column=i, pady=20)
        ...
        for hf, hp, tit, det in consultas:
            r = ctk.CTkFrame(scroll, fg_color="transparent"); r.pack(fill="x", pady=10)
    ...
    def criar_card_agendamento_detalhado(self, master, hora, tit, sub):
        c = ctk.CTkFrame(master, fg_color="white", corner_radius=25, border_width=1, border_color="#E2E8F0"); c.pack(fill="x", pady=5, padx=10)
