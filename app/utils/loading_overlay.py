...
# --- CLASSE DE CARREGAMENTO (Loading Overlay) ---
class LoadingOverlay:
    ...
    # --- CORREÇÃO: Método show_success adicionado dentro da classe ---
    def show_success(self, message="Sucesso!"):
        """Transforma o carregando em uma tela de sucesso elegante"""
        if not self.overlay:
            self.show(message)
            
        self.is_active = False # Para a animação do spinner
        self.spinner.configure(text="✔", text_color="#10B981") # Check verde
        self.label.configure(text=message, text_color="white")
        
        # Fecha automaticamente após um tempo curto
    ...
    def run_with_loading(self, func, message="Processando...", *args, **kwargs):
        ...
        def target():
            try:
                func(*args, **kwargs)
                # Note: O hide() aqui foi removido para que o _login_sucesso 
                # possa mostrar o Check verde antes de sumir.
            except Exception as e:
                # Se der erro, fecha o loading imediatamente
...
def run_with_loading(func, message="Processando...", *args, **kwargs):
    ...
    loading.run_with_loading(func, message, *args, **kwargs)
...
class VetAuthForm(ctk.CTkFrame):
    ...
    def tentar_entrar(self):
        ...
        if email_digitado == "" or senha_digitada == "":
            Modal(self, "Atenção", "Preencha os campos.", type="error")
            return

        run_with_loading(
            self._executar_login_com_loading, 
            "Autenticando...", 
            email_digitado, 
            senha_digitada
        )

    def _executar_login_com_loading(self, email_digitado, senha_digitada):
        try:
